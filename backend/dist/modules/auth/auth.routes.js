...
function buildResetLink(token) {
    const baseUrl = String(env_1.env.frontendUrl || "https://profe360cr.com").replace(/\/+$/, "");
...
async function getUserByCorreo(correo) {
    ...
    const result = await pool.request()
        .input("correo", database_1.sql.NVarChar, normalizeCorreo(correo))
        .query(`
      SELECT TOP 1
        u.UsuarioId,
        u.InstitucionId,
...
async function createPasswordResetToken(usuarioId) {
    ...
    await pool.request()
        .input("usuarioId", database_1.sql.Int, usuarioId)
        .input("tokenHash", database_1.sql.NVarChar, tokenHash)
        .input("expiresAt", database_1.sql.DateTime2, expiresAt)
        .query(`
...
router.post("/login", async (req, res) => {
    try {
        ...
        const passwordOk = await (0, password_1.comparePassword)(password, String(user.HashPassword || ""));
        if (!passwordOk) {
            return res.status(401).json({ ok: false, message: "Credenciales inválidas" });
        }
        const rolesResult = await (0, database_1.getPool)().then((pool) => pool.request()
            .input("usuarioId", database_1.sql.Int, user.UsuarioId)
            .query(`
          SELECT r.Nombre
          FROM dbo.UsuarioRol ur
...
router.post("/forgot-password", async (req, res) => {
    try {
        ...
        if (!user || !user.Activo || !user.NumeroCedula) {
            return (0, http_1.ok)(res, { enviado: true }, "Si el correo existe, se enviará un enlace de recuperación");
        }
...
router.post("/reset-password", async (req, res) => {
    try {
        ...
        if (!token || !correo || !numeroCedula || !newPassword) {
            return (0, http_1.badRequest)(res, "token, correo, numeroCedula y newPassword son obligatorios");
        }
        const tokenHash = hashResetToken(token);
        const pool = await (0, database_1.getPool)();
        const result = await pool.request()
...
Este correo es automático, por favor no responder.`
        ...
        return (0, http_1.ok)(res, { actualizado: true }, "Clave restablecida correctamente");
    }
    catch (error) {
