...
async function ensureRole(pool: any, usuarioId: number, roleName: string) {
  await pool.request().input("usuarioId", sql.Int, usuarioId).input("roleName", sql.NVarChar, roleName).query(`
...
async function createUser(pool: any, data: any) {
  const existing = await pool.request().input("correo", sql.NVarChar, data.correo).query(`SELECT TOP 1 UsuarioId FROM dbo.Usuario WHERE Correo = @correo`);
  if (existing.recordset[0]) return existing.recordset[0].UsuarioId;

  const hash = await hashPassword(data.password);
  const user = await pool.request()
    .input("institucionId", sql.Int, data.institucionId || null)
    .input("correo", sql.NVarChar, data.correo)
    .input("hash", sql.NVarChar, hash)
    .input("nombre", sql.NVarChar, data.nombre)
    .input("primerApellido", sql.NVarChar, data.primerApellido || null)
...
async function main() {
  ...
  const estudiantes = await pool.request().input("institucionId", sql.Int, institucionId).query(`SELECT COUNT(*) total FROM dbo.Estudiante WHERE InstitucionId = @institucionId`);
  if (estudiantes.recordset[0].total === 0) {
    await pool.request().input("institucionId", sql.Int, institucionId).query(`
  ...
  const estados = await pool.request().input("institucionId", sql.Int, institucionId).query(`SELECT COUNT(*) total FROM dbo.EstadoAsistencia WHERE InstitucionId = @institucionId`);
