...
import ReportesProfeGuia from "../components/ReportesProfeGuia";
import ComunicadosPanel from "../components/ComunicadosPanel";
import { prepararContextoNotas, calcularRegistroNotas, dedupeActividadesLogicas, dedupeSeguimientosActividadIndicadorLogico, agruparIndicadoresLogicos } from "../utils/registroNotas";
...
export default function GestionProfePage() {
  ...
  const consolidadoAlumnos = useMemo(() => calcularRegistroNotas(seguimientoContexto, asistenciaNotificaciones), [seguimientoContexto, asistenciaNotificaciones]);
...
function registrarPrimeraSeleccionAsistencia(estudianteId: number) {
  ...
  async function loadSeguimientoEvaluacion(item = selected, options: { incluirAsistencia?: boolean; incluirEnvios?: boolean; sincronizar?: boolean } = {}) {
    ...
    try {
      ...
      const data = prepararContextoNotas(dataRaw, item.GrupoClaseId && detalle?.estudiantes?.length ? new Set(detalle.estudiantes.map((e: any) => Number(e.EstudianteId))) : null);
        ...
        {!selected ? (
          ...
          <div style={{ display: "grid", gap: "16px", marginTop: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
              ...
              {!esGrupoGuia(selected) ? <button type="button" className={activePanel === "reportes" ? "primary-btn" : undefined} style={activePanel === "reportes" ? undefined : getGestionPanelButtonStyle("reportes")} onClick={() => { setActivePanel("reportes"); loadAsistencia(selected); loadSeguimientoEvaluacion(selected, { incluirAsistencia: true, incluirEnvios: true }); cargarAuditoriaEnvios(selected); cargarBoletasConductaReporte(); loadBitacora(selected); }}>Reportes</button> : null}
              {esGrupoGuia(selected) ? <button type="button" className={activePanel === "reportes_guia" ? "primary-btn" : undefined} style={getGestionPanelButtonStyle("reportes")} onClick={() => setActivePanel("reportes_guia")}>Reportes Profe Guía</button> : null}
              {!esGrupoGuia(selected) ? <button type="button" className={activePanel === "notas" ? "primary-btn" : undefined} style={activePanel === "notas" ? undefined : getGestionPanelButtonStyle("notas")} onClick={() => { setActivePanel("notas"); loadAsistencia(selected); loadSeguimientoEvaluacion(selected, { incluirAsistencia: true }); cargarCierreCurso(selected); }}>Registro de Notas</button> : null}
              {!esGrupoGuia(selected) ? <button
                type="button"
              ...
              <button type="button" className={activePanel === "comunicados" ? "primary-btn" : undefined} style={activePanel === "comunicados" ? undefined : getGestionPanelButtonStyle("comunicados")} onClick={() => setActivePanel("comunicados")}>Comunicados</button>
              {esGrupoGuia() ? (
                <button
                  type="button"
            ...
            {activePanel === "reportes_guia" && selected && esGrupoGuia(selected) ? (
              <ReportesProfeGuia key={getGrupoProfesorKey(selected)} guia={{ grupoId: selected.GrupoId, anioLectivoId: selected.AnioLectivoId, periodoId: selected.PeriodoId }} />
            ) : null}
            {activePanel === "comunicados" && selected ? <ComunicadosPanel key={getGrupoProfesorKey(selected)} grupo={selected} /> : null}
            {activePanel === "notas" && (
              <div style={{ display: "grid", gap: "14px", padding: "14px", border: "1px solid #cbd5e1", borderRadius: "16px", background: "#ffffff", color: "#0f172a" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
                  <div>
