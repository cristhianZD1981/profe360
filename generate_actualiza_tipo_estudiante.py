...
OUTPUT = Path(r"C:\Users\HP\OneDrive - Colegio de Profesionales en Informática y Comp\CURSOS ONLINE\PROFE360\actualiza_tipo_estudiante.sql")
...
def main() -> None:
    ...
    lines.append("")
    lines.append("-- Cambia este valor por el InstitucionId correcto antes de ejecutar.")
    lines.append("DECLARE @InstitucionId INT = NULL;")
    lines.append("")
    lines.append("IF @InstitucionId IS NULL")
    lines.append("BEGIN")
    lines.append("    RAISERROR('Debes asignar @InstitucionId antes de ejecutar el script.', 16, 1);")
    lines.append("    RETURN;")
    lines.append("END;")
    lines.append("")
    ...
    lines.append("")
    lines.append("-- Validar que existan los tipos de estudiante en el catalogo de la institucion.")
    ...
    lines.append("        ON te.InstitucionId = @InstitucionId")
    lines.append("       AND UPPER(LTRIM(RTRIM(te.Descripcion))) = UPPER(LTRIM(RTRIM(c.TipoEstudianteDescripcion)))")
    ...
    lines.append("    WHERE te.TipoEstudianteId IS NULL;")
    lines.append("")
    lines.append("    RAISERROR('Hay tipos de estudiante que no existen en dbo.TipoEstudiante para esa institucion.', 16, 1);")
    ...
    lines.append("    ON te.InstitucionId = @InstitucionId")
    lines.append("   AND UPPER(LTRIM(RTRIM(te.Descripcion))) = UPPER(LTRIM(RTRIM(c.TipoEstudianteDescripcion)))")
    lines.append("WHERE e.InstitucionId = @InstitucionId;")
    lines.append("")
    lines.append("SELECT")
    lines.append("    @@ROWCOUNT AS FilasActualizadas,")
    lines.append("    (SELECT COUNT(1) FROM @Cambios) AS FilasEnExcel,")
    lines.append("    (SELECT COUNT(1) FROM dbo.Estudiante e INNER JOIN @Cambios c ON REPLACE(REPLACE(LTRIM(RTRIM(e.Identificacion)), N' ', N''), N'-', N'') = REPLACE(REPLACE(LTRIM(RTRIM(c.Identificacion)), N' ', N''), N'-', N'') WHERE e.InstitucionId = @InstitucionId) AS CoincidenciasEstudiante;")
    lines.append("")
