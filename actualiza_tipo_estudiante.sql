...
-- Cambia este valor por el InstitucionId correcto antes de ejecutar.
DECLARE @InstitucionId INT = NULL;

IF @InstitucionId IS NULL
BEGIN
    RAISERROR('Debes asignar @InstitucionId antes de ejecutar el script.', 16, 1);
    RETURN;
END;
...
-- Validar que existan los tipos de estudiante en el catalogo de la institucion.
IF EXISTS (
    SELECT 1
    FROM (SELECT DISTINCT TipoEstudianteDescripcion FROM @Cambios) c
    LEFT JOIN dbo.TipoEstudiante te
        ON te.InstitucionId = @InstitucionId
       AND UPPER(LTRIM(RTRIM(te.Descripcion))) = UPPER(LTRIM(RTRIM(c.TipoEstudianteDescripcion)))
    WHERE te.TipoEstudianteId IS NULL
)
BEGIN
    SELECT DISTINCT c.TipoEstudianteDescripcion AS TipoFaltante
    FROM (SELECT DISTINCT TipoEstudianteDescripcion FROM @Cambios) c
    LEFT JOIN dbo.TipoEstudiante te
        ON te.InstitucionId = @InstitucionId
       AND UPPER(LTRIM(RTRIM(te.Descripcion))) = UPPER(LTRIM(RTRIM(c.TipoEstudianteDescripcion)))
    ...
    RAISERROR('Hay tipos de estudiante que no existen en dbo.TipoEstudiante para esa institucion.', 16, 1);
    ROLLBACK TRAN;
    RETURN;
END;
...
SELECT
    ...
    (SELECT COUNT(1) FROM dbo.Estudiante e INNER JOIN @Cambios c ON REPLACE(REPLACE(LTRIM(RTRIM(e.Identificacion)), N' ', N''), N'-', N'') = REPLACE(REPLACE(LTRIM(RTRIM(c.Identificacion)), N' ', N''), N'-', N'') WHERE e.InstitucionId = @InstitucionId) AS CoincidenciasEstudiante;
